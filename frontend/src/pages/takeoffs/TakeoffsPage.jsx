...
export default function TakeoffsPage({ projectId }) {
  ...
  const loadData = async () => {
    ...
    try {
      const [sheetsRes, assembliesRes] = await Promise.all([
        api.get(`/api/v1/takeoffs/project/${projectId}`),
        api.get('/api/v1/takeoffs/assemblies'),
      ]);
      setSheets(sheetsRes || []);
  ...
  const loadSheet = async (sheetId) => {
    try {
      const sheet = await api.get(`/api/v1/takeoffs/sheets/${sheetId}`);
...
function TotalsFooter({ sheetId }) {
  ...
  const loadTotals = async () => {
    try {
      const data = await api.get(`/api/v1/takeoffs/sheets/${sheetId}/totals`);
      setTotals(data);
    } catch (error) {
...
function NewSheetModal({ projectId, onSave, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      const sheet = await api.post(`/api/v1/takeoffs/project/${projectId}`, form);
      onSave(sheet);
    } catch (error) {
...
function AddItemModal({ sheetId, assemblies, onSave, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      await api.post(`/api/v1/takeoffs/sheets/${sheetId}/items`, {
        ...form,
