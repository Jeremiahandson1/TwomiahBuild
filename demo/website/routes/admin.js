...
initFile(homepageFile, {
  ...
  ctaSection: {
    ...
    primaryButtonLink: '#contact',
    secondaryButtonText: 'Call (555) 123-4567',
...
<p>We serve Austin and the surrounding Greater Austin Area area.</p>
...
const trackPageView = (pageId, referrer = '', userAgent = '') => {
  try {
    ...
    Object.keys(analytics.dailyViews).forEach(date => {
      if (date < cutoffStr) delete analytics.dailyViews[date];
    });
...
router.get('/analytics', authMiddleware, (req, res) => {
  try {
    ...
    const stats = {
      totalPageViews: Object.values(analytics.pageViews).reduce((sum, p) => sum + p.total, 0),
      todayPageViews: analytics.dailyViews[today] || 0,
      weekPageViews: Object.entries(analytics.dailyViews)
        .filter(([date]) => date >= lastWeek)
        .reduce((sum, [, count]) => sum + count, 0),
      totalLeads: leads.length,
      newLeads: leads.filter(l => l.status === 'new').length,
      topPages: Object.entries(analytics.pageViews)
        .map(([page, data]) => ({ page, views: data.total }))
        .sort((a, b) => b.views - a.views)
        .slice(0, 10),
      dailyViews: Object.entries(analytics.dailyViews)
...
router.post('/track', (req, res) => {
  ...
  if (pageId) {
    trackPageView(pageId, referrer, userAgent);
  }
  res.json({ ok: true });
});
