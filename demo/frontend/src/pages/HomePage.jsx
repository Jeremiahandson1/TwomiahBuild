...
// Scroll animations
  ...
  const zip = settings?.zip || '78701';
  
  // Homepage sections
  const hero = homepage?.hero || {};
  const trustBadges = homepage?.trustBadges?.filter(b => b.enabled) || [];
  const ctaSection = homepage?.ctaSection || {};
  const serviceAreas = homepage?.serviceAreas || ['Austin', 'Pflugerville', 'Round Rock', 'Cedar Park', 'Georgetown'];
    ...
    <div className="home-page">
      ...
      >
        ...
        <div className={`hero-video-content ${hero.image ? 'hero-content' : ''}`}>
          <h1>{hero.title || 'Austin Area Contractor'}</h1>
          ...
          <p className="hero-awards">{hero.description || 'Serving the Greater Austin Area with Quality Craftsmanship'}</p>
