...
router.post('/webhook', async (req, res) => {
  ...
  try {
    // Use raw body for signature verification (preserved by express.json verify callback)
    const event = stripeService.constructWebhookEvent(req.rawBody || req.body, signature);

    // Try Factory webhook handler first (operator billing)
    ...
    if (factoryResult.handled) {
      ...
      if (factoryResult.updates) {
        const where = factoryResult.factoryCustomerId
          ? { id: factoryResult.factoryCustomerId }
          : factoryResult.lookupField
            ? { [factoryResult.lookupField]: factoryResult.lookupValue }
            : null;
        ...
        if (where) {
          try {
            await prisma.factoryCustomer.update({
              where,
              data: factoryResult.updates,
            });
            console.log('Updated FactoryCustomer:', where);
          } catch (dbErr) {
            console.error('Failed to update FactoryCustomer:', dbErr.message);
          }
        }
      }

      return res.json({ received: true, ...factoryResult });
    }
