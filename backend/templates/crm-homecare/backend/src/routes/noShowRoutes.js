    ...
    for (const row of overdue.rows) {
      ...
      if (config.notify_caregiver && row.caregiver_phone) {
        smsSent = await sendSMS(row.caregiver_phone,
          `Alert: You were scheduled to start with ${row.client_name} at ${row.expected_start}. Please clock in or contact the office immediately.`
        );
      }
      ...
      if (config.notify_admin && config.admin_phone) {
        await sendSMS(config.admin_phone,
          `No-Show Alert: ${row.caregiver_name} has not clocked in for ${row.client_name} (scheduled ${row.expected_start}). Please follow up.`
        );
      }
